// ==UserScript==
// @name           jx-letterboxd-common-films-b
// @version        0.0.1
// @namespace      https://github.com/JenieX/user-js
// @description    The bundle version of the script by the same name.
// @author         JenieX
...
}(unsafeWindow, (function (exports) {  function getWindow(node) {
    if (node == null) {
      return window;
    }
...
const noCommonFilms = 'You have no <strong>common</strong> films with this user';
var messages = {
    loading,
    isYou,
    noCommonFilms,
...
function getMyAccount() {
    // Cookies.set('letterboxd.signed.in.as', 'farid75752');
    const myAccount = Cookies.get('letterboxd.signed.in.as');
    console.log({ myAccount });
    if (myAccount === undefined) {
        throw new Error(`${LOG_ID} You must be logged in to use this user script`);
    }
    return myAccount;
}
...
function extractFilmData(posterElement) {
    const name = $('img', posterElement).alt;
...
async function fetchFilms({ link, collector }) {
...
async function fetchMyFilms({ link, collector }) {
    const films = collector ?? [];
    ...
    const posterElements = $$('li[class="poster-container"]', container);
    for (const posterElement of posterElements) {
        films.push(extractFilmData(posterElement));
    }
    const nextPageElement = documentX.querySelector('.paginate-nextprev > a.next');
    if (nextPageElement === null) {
        return films;
...
async function fetchWatchlistFilms({ link, collector }) {
...
function clearExpired() {
    ...
    for (const userKey of usersInStorage) {
        ...
        const { expires } = JSON.parse(commonFilmsData);
...
function getCommonFilms(user) {
    const commonFilmsData = localStorage.getItem(`common-films-${user}`);
    if (commonFilmsData === null) {
        return undefined;
    }
    const { expires, films, total } = JSON.parse(commonFilmsData);
    const currentTimestamp = Date.now();
    if (currentTimestamp > expires) {
        console.warn(LOG_ID, `Stored data for "${user}" got expired, fetching fresh data..`);
        return undefined;
    }
    const commonFilms = films;
...
function setCommonFilms(user, commonFilms) {
...
async function rate({ id, rating, name }) {
...
async function markAsWatched({ id, name }) {
    await rate({ id, rating: 0, name });
}
async function markAsNotWatched({ id }) {
    const myFilms = getFilms();
    if (myFilms === undefined) {
        return;
    }
...
const detectTypes = new Set([
    'mark-as-not-watched',
    'mark-as-watched',
    'rate',
]);
function XHROpenOverride(...args) {
    const instance = this;
    const [, url] = args;
    ...
    instance.addEventListener('readystatechange', async () => {
        if (instance.readyState === XMLHttpRequest.DONE) {
            const { result, rating, film: { name }, } = JSON.parse(instance.responseText);
            if (result !== true) {
                return;
            }
            switch (type) {
...
function getFilmPoints(myRating, userRating) {
...
const labels = ['way-off', 'off', 'close', 'match', 'great-match'];
function evaluateFilms({ commonFilms, sortByName, filter }) {
    const myFilms = getFilms();
    let totalCommon = 0;
    let perfectPoints = 0;
    let userPoints = 0;
    const evaluatedFilms = Object.assign([], { match: 0, common: 0 });
...
function createBodyCell(iconClass) {
 ...
 */
function setContent$1({ rowElement, match, common }) {
...
function prepareContent() {
    ...
    for (const rowElement of rowElements) {
        const avatarElement = $('a.avatar', rowElement);
        ...
        if (commonFilms !== undefined) {
            ...
            setContent$1({ rowElement, match, common });
        }
    }
}
 ...
 * Sets the content of match and common elements in the table.
 */
function setContent({ rowElement, match, common }) {
    const matchElement = $('.icon-link', rowElement);
    matchElement.lastChild.textContent = `${match}%`;
    const commonElement = $('.icon-common', rowElement);
    commonElement.lastChild.textContent = common.toString();
...
async function modifyTable() {
    addStyle('section.col-17.col-main{width:100%}.icon-common{padding-left:24px}.icon-common>.icon{background-position:-410px -90px;height:16px;width:20px}.person-table .table-stats{width:unset!important}.person-table .table-person{width:35%!important}.cols-2>.sidebar{display:none}');
...
async function prepareTooltip(instance, filter) {
    ...
    const commonFilmsText = createTooltipContent({
        evaluatedFilms,
        user,
        totalFilms,
    });
...
async function attachTooltip(element) {
    // console.log(element);
    tippy(element, {
...
function createSimilarTooltip(similarFilms, filmID) {
    ...
    for (const [id, name] of similarFilms) {
        ...
        if (id === filmID) {
            listItem.firstElementChild.style.setProperty('color', '#00E054');
        }
    }
    return container.outerHTML;
}

function getSimilarFilms(targetRating, myFilms) {
    const similarFilms = [];
...
async function attachSimilarRatingTooltip() {
...
async function main() {
    ...
    try {
        ...
        for (const avatarsElement of avatarsElements) {
            attachTooltip(avatarsElement).catch(() => { });
        }
    }
    catch { }
}
 ...
 */
if (!/letterboxd\.com\/[^/]+\/films\//.test(TAB_URL)) {
...
addStyle('#common-films>header{cursor:default;margin-bottom:10px}#common-films-match>h3{color:#939393;font-size:medium;font-weight:bolder;text-align:center}#common-films-total{color:#535353;text-align:center}#common-films-list{max-height:50vh;overflow:auto;width:300px}#common-films-list>li{padding:7px}#common-films-list>:not(:last-child){border-bottom:1px solid #353535}#common-films-list>li.similar{text-align:left}#common-films-list>li.great-match>a{color:#15b3e9}#common-films-list>li.match>a{color:#8f6be2}#common-films-list>li.close>a{color:#35d274}#common-films-list>li.off>a{color:#dd8820}#common-films-list>li.way-off>a{color:#e94363}#common-films-list>li.not-rated>a,#common-films-list>li.similar>a{color:#7b7b7b}#common-films-list>li.watchlisted>a{color:#a5a5a5}#common-films-list::-webkit-scrollbar{height:3px;width:3px}#common-films-list::-webkit-scrollbar-thumb{background:#353535}.tippy-box{background-color:#000}.tippy-arrow{color:#000}');
addStyle('#film-popular-lists,#find-friends,.account-status,.block-flag-wrapper,.film-news,.js-hide-in-app.banner-230.banner,.js-hide-in-app.banner-250.banner,.js-hide-in-app.banner-950.banner,.page-footer,.panel-sharing,.panel-text.section,.watch-panel{display:none}');
clearExpired();
// ------------------------
const matchesWideTablePage = (/letterboxd\.com\/[^/]+\/(following|followers)\//.test(TAB_URL) ||
    /letterboxd\.com\/[^/]+\/friends\/film\//.test(TAB_URL) ||
    /letterboxd\.com\/film\/[^/]+\/(likes|members|fans)\//.test(TAB_URL));
